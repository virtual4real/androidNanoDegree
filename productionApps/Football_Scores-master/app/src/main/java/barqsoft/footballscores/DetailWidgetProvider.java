...
public class DetailWidgetProvider extends AppWidgetProvider {
    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
        ...
        for (int appWidgetId : appWidgetIds) {
            RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.detail_appwidget);
    ...
    private void setRemoteAdapterV11(Context context, @NonNull final RemoteViews views, int nWidgetId) {
        Intent intent = new Intent(context, DetailWidgetRemoteViewsService.class);
        intent.putExtra("WIDGET_TITLE", "bla bla");
        intent.putExtra("WIDGET_LEAGUE", "ss");
        context.startService(intent);


        views.setRemoteAdapter(nWidgetId, R.id.widget_list, intent);
    }
